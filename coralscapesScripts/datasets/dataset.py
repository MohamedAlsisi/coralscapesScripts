...
class Coralscapes(Dataset):
    ...
    def __init__(self, root = "../../coralscapes", split='train', transform=None, transform_target=True):
        ...
        # If any item in images_dir is a directory, assume nested (Cityscapes/Coralscapes style)
        items = [i for i in os.listdir(self.images_dir) if not i.startswith('.')]
        is_nested = any(os.path.isdir(os.path.join(self.images_dir, d)) for d in items)
        ...
        if is_nested:
            for city in items:
                ...
                for file_name in os.listdir(img_dir):
                    ...
                    # Standard replacement for nested structure
                    target_file_name = file_name.replace("leftImg8bit", "gtFine")
                    self.targets.append(os.path.join(target_dir, target_file_name))
        else:
            # Flat structure (Sponges style)
            for file_name in items:
                if not (file_name.lower().endswith(('.png', '.jpg', '.jpeg'))):
                    continue
                self.images.append(os.path.join(self.images_dir, file_name))
                
                # Robust target discovery for flat structure
                # Logic 1: name.jpg -> name_labelIds.png (Sponges)
                # Logic 2: name_leftImg8bit.png -> name_gtFine_labelIds.png (Coralscapes flat)
                base_name, _ = os.path.splitext(file_name)
                if "leftImg8bit" in base_name:
                    target_file_name = base_name.replace("leftImg8bit", "gtFine_labelIds") + ".png"
                else:
                    target_file_name = f"{base_name}_labelIds.png"
