...
def get_windows_inference(inputs, window_size, stride):
    ...
    for y in range(0, height, stride):
        ...
        for x in range(0, width, stride):
            ...
            # Pad if the window is at the boundary and smaller than window_size
            if input_window.shape[-2] < window_size or input_window.shape[-1] < window_size:
                pad_h = window_size - input_window.shape[-2]
                pad_w = window_size - input_window.shape[-1]
                input_window = torch.nn.functional.pad(input_window, (0, pad_w, 0, pad_h))
...
def get_windows(batch, window_size, stride, window_size_label = None, stride_label = None):
    ...
    for y in range(0, height, stride):
        ...
        for x in range(0, width, stride):
            ...
            # Pad if boundary windows are too small
            if input_window.shape[-2] < window_size or input_window.shape[-1] < window_size:
                pad_h = window_size - input_window.shape[-2]
                pad_w = window_size - input_window.shape[-1]
                input_window = torch.nn.functional.pad(input_window, (0, pad_w, 0, pad_h))
                # For labels, we use 0 or 255 depending on ignore_index. 
                # Assuming 0 is often used as background/ignored in some parts of this codebase
                # or we just use 255 if that's the current target.
                # To be safe, we'll use 0 here as it's the most common default in this lib's logic for "unlabeled"
                label_window = torch.nn.functional.pad(label_window, (0, pad_w, 0, pad_h), value=0)
    ...
    if(window_size_label is not None and stride_label is not None):
        ...
        for y in range(0, height, stride_label):
            ...
            for x in range(0, width, stride_label):
                ...
                if label_window.shape[-2] < window_size_label or label_window.shape[-1] < window_size_label:
                    pad_h = window_size_label - label_window.shape[-2]
                    pad_w = window_size_label - label_window.shape[-1]
                    label_window = torch.nn.functional.pad(label_window, (0, pad_w, 0, pad_h), value=0)
                label_windows_row.append(label_window)
            label_windows.append(label_windows_row)
