...
def get_loss_fn(loss_name, loss_params):
    ignore_index = loss_params.pop('ignore_index', 0)
    if loss_name == 'cross_entropy':
        return torch.nn.CrossEntropyLoss(ignore_index = ignore_index, **loss_params)
    elif loss_name == 'dice':
        return smp.losses.DiceLoss(mode = "multiclass", ignore_index = ignore_index, **loss_params)
    elif loss_name == 'cross_entropy+dice':
        return CombinedCrossEntropyDiceLoss(ignore_index = ignore_index, **loss_params)
