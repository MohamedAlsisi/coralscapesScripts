...
def get_batch_predictions(data, model, device, loss_fn = None):
    ...
    if("pixel_values" in data and "mask_labels" in data):
        outputs = model(
    ...
    if("pixel_values" in data and "labels" in data):
        outputs = model(
            pixel_values=data["pixel_values"].to(device),
...
class Benchmark_Run:
    ...
    def __init__(self, run_name: str, model_name: str, N_classes: int, 
        ...
        if(model_name == "deeplabv3+resnet50"):
            ...
            ignore_index = training_hyperparameters.get("loss", {}).get("ignore_index", 0)
            self.model = SegformerForSemanticSegmentation.from_pretrained(
                pretrained_model_name, id2label={i:i for i in range(0, N_classes)}, 
                semantic_loss_ignore_index = ignore_index, ignore_mismatched_sizes=True
            )
        ...
        if(model_checkpoint):
            ...
            if("model_state_dict" in checkpoint):
                ...
                self.model.load_state_dict(torch.load(model_checkpoint), strict=False)
