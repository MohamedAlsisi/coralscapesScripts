"""
Evaluate saved coralscapes Lv1 predictions against ground-truth label masks.

Usage (from coralscapesScripts/ project root):

    python scripts/coralscapes_evaluate_lv1.py \\
        --preds   work_dir/coralscapes_lv1_pred_<timestamp> \\
        --labels  coralscapes_lv1/gtFine/test \\
        [--output work_dir/.../eval_report.csv] \\
        [--ignore-index 0]

Outputs:
    eval_report_lv1.csv          – per-image metrics
    eval_report_lv1_summary.txt  – human-readable summary with per-class confusion detail
    confusion_matrix_lv1.csv     – full N×N confusion matrix
"""
...
def top_confusions(cm: np.ndarray, cls_id: int, id2label: dict,
    ...
    row = cm[cls_id].copy()
    row[cls_id]       = 0   # exclude correct predictions
    row[ignore_index] = 0   # exclude ignore class
    total = cm[cls_id].sum()
    if total == 0:
        return []
    results = []
...
# ─── CLI ─────────────────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate coralscapes Lv1 segmentation predictions")
...
def main():
    ...
    output_csv = args.output or os.path.join(pred_dir, "eval_report_lv1.csv")
    output_txt = output_csv.replace(".csv", "_summary.txt")
    output_cm  = os.path.join(os.path.dirname(output_csv), "confusion_matrix_lv1.csv")
    ...
    lines = []
    lines.append("=" * W)
    lines.append("  CORALSCAPES LV1 — PER-CLASS EVALUATION REPORT")
    lines.append("=" * W)
    ...
    # Sort worst → best by IoU so problem classes are immediately visible
    ...
    for c in class_ids:
        ...
        # Show top-3 confusions (what the model predicted instead)
