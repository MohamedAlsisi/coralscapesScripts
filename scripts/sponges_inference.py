"""
Inference script for the fine-tuned SegFormer on Sponges_18167_sp10.

Usage (from coralscapesScripts/ project root):
    python scripts/sponges_inference.py \
        --inputs  Sponges/Sponges_18167_sp10/leftImg8bit/test \
        --config  configs/segformer-mit-b2-sponges_sp10.yaml \
        --model-checkpoint model_checkpoints/segformer_mit_b2-sponges_sp10/model_epoch4_final \
        [--outputs work_dir/sponges_predictions]

Outputs per image (inside --outputs dir):
    <name>_pred.png      – raw integer label mask (for quantitative evaluation)
...
# ─── Sponges class/colour mapping ────────────────────────────────────────────
# Class 0 = unlabeled/background (always present in Coralscapes dataset class)
id2label = {
    0: "unlabeled",
...
id2color = {
    0: [255, 255, 255],   # unlabeled → white
    1: [255, 255, 0],     # Ball Yellow Papillate Irregular
    2: [245, 245, 220],   # Cup Beige Thick
    3: [50,  50,  50],    # Cup Black Smooth
    4: [255, 165, 0],     # Cup Orange
    5: [255, 0,   0],     # Cup Red Smooth
    6: [139, 0,   0],     # Cup Red Thick
    7: [255, 215, 0],     # Cup Yellow
    8: [255, 192, 203],   # Fan Pink
    9: [128, 0,   128],   # Massive Purple
}

N_CLASSES = len(id2label)  # 10
...
if args.outputs:
    ...
    output_dir = os.path.join(work_dir, f"sponges_pred_{timestamp_str}")
...
def label_to_color(label_arr):
    """Convert H×W integer label array → H×W×3 uint8 colour array."""
    h, w = label_arr.shape
    color_arr = np.zeros((h, w, 3), dtype=np.uint8)
    for class_id, color in id2color.items():
        mask = (label_arr == class_id)
...
for i, image_path in enumerate(image_paths):
    ...
    label_pred_np = np.array(label_pred)
    ...
    # Mirror directory structure inside output_dir
    if os.path.isfile(input_dir):
...
with open(log_path, "w") as f:
    f.write("=" * 60 + "\n")
    f.write("        SPONGES INFERENCE EXPERIMENT LOG\n")
    f.write("=" * 60 + "\n\n")
    ...
    for k, v in id2label.items():
        color_str = str(id2color[k])
        f.write(f"  {k:>2}: {v:<40} {color_str}\n")
    f.write("\n" + "=" * 60 + "\n")
