"""
Inference script for a fine-tuned SegFormer trained on coralscapes_lv1
(14-class Level-1 hierarchy).

Usage (from coralscapesScripts/ project root):
    python scripts/coralscapes_inference_lv1.py \\
        --inputs  coralscapes/leftImg8bit/test \\
        --config  configs/segformer-mit-b2-lv1.yaml \\
        --model-checkpoint <path/to/checkpoint> \\
        [--outputs work_dir/coralscapes_lv1_predictions]
...
id2color = {
    ...
    7:  [226, 91,  157],   # Coral Alive
    8:  [252, 231, 240],   # Coral Bleached
    9:  [114, 60,  61],    # Coral Dead
    10: [194, 178, 128],   # Sand
    11: [125, 222, 125],   # Seagrass
    12: [8,   205, 12],    # Transect Tools
    13: [255, 0,   134],   # Trash
    14: [125, 125, 125],   # Hard Substrate
}

N_CLASSES = len(id2label)  # 15 (0..14)
...
if args.outputs:
    ...
    output_dir = os.path.join(work_dir, f"coralscapes_lv1_pred_{timestamp_str}")
...
with open(log_path, "w") as f:
    f.write("=" * 60 + "\n")
    f.write("    CORALSCAPES LV1 INFERENCE EXPERIMENT LOG\n")
    f.write("=" * 60 + "\n\n")
    ...
    f.write("─" * 60 + "\n")
    f.write("CLASS MAPPING (LV1)\n")
    f.write("─" * 60 + "\n")
    for k, v in id2label.items():
        f.write(f"  {k:>3}: {v:<38} {str(id2color[k])}\n")
    f.write("\n" + "=" * 60 + "\n")
